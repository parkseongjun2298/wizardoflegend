...
int CFireCard::Update()
{
	if (m_bDie)
	{ 
		if(m_FireUi)
		{ 
		CFireSkillbar* pFireCard = CFireSkillbar::Create();
...
void CFireCard::Render()
{
	const TEX_INFO* pTexInfo = m_pTextureMgr->GetTexInfo(L"Ui", L"FIRE_DRAGON_CARD", (int)m_fFrameStart);
	NULL_CHECK(pTexInfo);
...
void CFireCard::Release()
{
}

CFireCard * CFireCard::Create(float _X, float _Y)
{
	CFireCard* pInstance = new CFireCard;
