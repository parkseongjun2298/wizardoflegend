...
void CPrisonVer::Render()
{
	const TEX_INFO* pTexInfo = m_pTextureMgr->GetTexInfo(L"Prison", L"Ver", (int)m_fFrameStart);
	NULL_CHECK(pTexInfo);

	float fCenterX = pTexInfo->tImgInfo.Width * 0.5f;
	float fCenterY = pTexInfo->tImgInfo.Height * 0.4f;
...
HRESULT CPrisonVer::Initialize()
{
	m_tInfo.vSize = { 1.f, 1.f, 0.f };
	m_tInfo.vPictureSize = { 23,427,0 };
...
void CPrisonVer::Release()
{
}

CPrisonVer * CPrisonVer::Create(float _X, float _Y)
{
	CPrisonVer* pInstance = new CPrisonVer;
